<template>
  <f7-page :name="$route.route.name" :class="$route.route.name">
    ...
    <f7-list class="searchbar-found" id="search-list">
      <f7-list-item v-for="templet in userList" :title="templet.name"
                    :link="'/admin/templet-up/'+templet.templetId" :after="templet.departmentAndProportion" swipeout @swipeout:deleted="deleteProject(templet.templetId)">
          <f7-swipeout-actions>
  ...
  export default {
    ...
    methods: {
      deleteProject: function (fId) {
        ...
        if (fId > 0) {
          window.f7.confirm('确认删除该模板吗?', '操作提醒', function () {
            ...
            util.svcInvoke('service', 'templet/deTemplet', period, function (result) {
              util.alert('删除成功!')
            })
          },function () {
      ...
      getUserList: function () {
        let $this = this
        util.svcInvoke('service', 'templet/seTemplet', null, function (result) {
          // 关闭窗口并刷新主页面
